...
class QAEngine:
    ...
    def _extract_reference_point(self, rtss: pydicom.Dataset) -> Optional[Dict[str, Any]]:
        ...
        # 2. Look for ROIs named 'REFERENCE POINT' or 'ISOCENTER'
        for roi_contour in rtss.ROIContourSequence:
            roi_name = roi_map.get(roi_contour.ReferencedROINumber, "")
            if "REFERENCE POINT" in roi_name or "ISOCENTER" in roi_name:
                # If it's a point (Point ROI), it might be in ContourSequence
